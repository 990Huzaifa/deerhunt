...
class StoreAsRecentPostJob implements ShouldQueue
{
    ...
    public function handle(NotificationService $notificationService): void
    {
        ...
        $image = $this->payload['image'] ?? null;
        $allImagePaths = [];

        if (!empty($this->payload['images']) && is_array($this->payload['images'])) {
            $allImagePaths = array_values(array_filter($this->payload['images']));
            $image = !empty($allImagePaths) ? implode(',', $allImagePaths) : null;
        } elseif (!empty($image)) {
            $allImagePaths = [$image];
        }
