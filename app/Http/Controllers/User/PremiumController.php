...
class PremiumController extends Controller
{
    ...
    public function verifygoogle(Request $request): JsonResponse
    {
        try{
            ...
            $checkSub = $user->premium()->where('platform', 'google')->where('user_id', $user->id)->first();
            $caseData = 'new';
            if ($checkSub) {
                $caseData = 'upgrade';
            }
            ...
            Log::info('Premium Google Verification Data: ' . json_encode($verificationData));
    ...
    private function verifyFromGoogle($token, $parent_id)
    {
        ...
        if (!$accessToken) {
            Log::error(' Premium Cannot proceed, Google Access Token not available.');
            return null;
        }
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                Log::warning("Premium Google Verification Failed: " . $response->body());
                return  null;
            }
        } catch (Exception $e) {
            // Connection/Guzzle error
            Log::error("Premium HTTP Request Error to Google: " . $e->getMessage());
            return $e->getMessage();
        }
    }
    ...
    public function verifyapple(Request $request): JsonResponse
    {
        try{
            ...
            $checkSub = $user->premium()->where('platform', 'apple')->orderBy('updated_at', 'desc')->first();
            $caseData = 'new';
            if ($checkSub) {
                $caseData = 'upgrade';
            }
            ...
            if($caseData == 'upgrade'){
                DB::transaction(function () use ($user, $plan, $productId, $expiresAt, $latestReceipt) {
                    $user->premium()->where('transaction_id', $latestReceipt['originalTransactionId'])->update([
                            'plan'           => $productId,
                            'status'         => 'active',
                            'platform'          => "apple",
                            'expires_at'     => $expiresAt,
